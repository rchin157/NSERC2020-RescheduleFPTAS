...
*/
	...
	determineS(para, &sValues, delta);

	//run reschedule with all s value combos and return best
	State *minState = NULL;
	int combo[para[NUMMACHINES]];
	//printf("Running combinations...\n");
	runCombinations(sValues, combo, 0, vl[0] - 1, 0, para[NUMMACHINES], jobs, para, &minState, delayed, vl, delta);
 ...
 *********************************************************************************************************************************/
void runCombinations(int *sValues, int combo[], int start, int end, int index, int m, int **jobs, int *para, State **min, int *d, int *vl, float delta){
...
/*		printf("combo: ");
		...
		State *temp = reschedule(jobs, para, combo, vl, delta);
	...
	for(int i = start; i <= end; i++){
		...
		runCombinations(sValues, combo, i, end, index + 1, m, jobs, para, min, d, vl, delta);
	}
}
